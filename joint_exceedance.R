load("~/GitHub/SpatExtreme/MCMC_HUC02.RData")
library(fields)
library(torch)
library(SPQR)
...
stn <- read.csv("dataforthepaper/HCDN-2009_Station_Info.csv")
...
mean(exceedances[,4]-exceedances[,3]>0,na.rm = T)
sd(exceedances[,4]-exceedances[,3]>0,na.rm = T)
summary(exceedances[,4])
stn2 = stn[nmiss==0,]
stn2[cluster[1:7],]
mean(exceedances[,3]);sd(exceedances[,3])
