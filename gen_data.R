...
create_locations_evp <- function(k=10,n=50,seed=1,min_dist=0.025){
    ...
    nn_dist[nn_dist==Inf] <- 10 #change this to lower value
    for(i in 2:n){
...
create_locations_jittered <- function(NS=10,m=100,seed=1){
    rs <- .Random.seed 
    set.seed(seed)
    locs_x      <- rep(seq(0.05,0.95,.1),each = 10) + rnorm(m,0,.01)
    locs_y    <- rep(seq(0.05,0.95,.1),10) + rnorm(m,0,.01)
    locs    <- cbind(locs_x,locs_y)
    locs       <- locs[order(rowSums(locs)),]
...
create_locations_gridded <- function(NS=10,m=100,seed=1){
  rs <- .Random.seed 
  set.seed(seed)
  if(m==400){
    locs_x       <- rep(seq(0.05,1.95,.1),each = 20)
    locs_y    <- rep(seq(0.05,1.95,.1),20)  
  } else{
  locs_x       <- rep(seq(0.05,0.95,.1),each = 10)
  locs_y    <- rep(seq(0.05,0.95,.1),10)
  }
  locs    <- cbind(locs_x,locs_y)
  # locs       <- locs[order(rowSums(locs)),]
  nn_dist    <- array(Inf,c(NS,2,m))
  ...
  for(i in 2:m){
    ...
    if(i>(NS+1)){
      dtrunc        <- ifelse(1:m <i,dd[i,],Inf)
      nn_id[i,]     <- which(rank(dtrunc,ties.method = 'f')<=NS)
      nn_dist[,1,i] <- locs[nn_id[i,],1]-locs[i,1]
      nn_dist[,2,i] <- locs[nn_id[i,],2]-locs[i,2]
    }
  }
...
gen_data_evp_local <- function(n,s,min_alpha=1,max_alpha=2,min_rho=0.01,max_rho=1,min_delta=0,max_delta=1,verbose=F){
    m       <- nrow(s) 
...
# gen_data_evp_local3 <- function(n,s,min_rho=0.01,max_rho=0.50,min_delta=0.01,max_delta=0.99,min_r = 0.01, max_r = 0.99, reps=1){
#     n      <- n/reps
#     rho    <- runif(n,min_rho,max_rho)
#     delta  <- runif(n,min_delta,max_delta)
...
#     DELTA  <- NULL
#     for(i in 1:n){
#         br_local <- t(rmaxstab(reps, s, "brown", range = rhoW2rhoR(rho[i],1), smooth = 1))
#         gev_mar <- matrix(rgev(nrow(s), 1, 1, 1), ncol = 1)
#         tmp = cbind(r[i]*br_local,(1-r[i])*gev_mar)
#         R_s = apply(tmp,1,max)
#         gp_local <- t(rgp(reps, s, "powexp", nugget = r[i], range = rho[i], smooth = 1))
#         br_exp   <- br_to_exp(R_s)
#         gp_exp   <- gp_to_exp(gp_local,nugget = r[i])
#         y        <- delta[i]*br_exp + (1-delta[i])*gp_exp
#         y        <- phypoexp2(y, rate1 = 1/delta[i], rate2 = 1/(1-delta[i]))
#         Y        <- cbind(Y,y)
#         RHO      <- c(RHO,rep(rho[i],reps))
#         DELTA    <- c(DELTA,rep(delta[i],reps))
#     }
#     out <- list(Y=Y,rho=RHO,delta=DELTA)
#     return(out)}

gen_data_evp_local3 <- function(n,s,min_rho=0.01,max_rho=0.50,min_delta=0.01,max_delta=0.99,min_r = 0.01, max_r = 0.99){
    rho    <- runif(n,min_rho,max_rho)
    delta  <- runif(n,min_delta,max_delta)
    r       <- runif(n, min_r, max_r)
...
gen_data_mspgp_local <- function(n,s,min_rho=0.01,max_rho=0.50,min_delta=0.01,max_delta=0.99,reps=1){
    n      <- n/reps
    rho    <- runif(n,min_rho,max_rho)
    delta  <- runif(n,min_delta,max_delta)
    Y      <- NULL
    ...
    DELTA  <- NULL
    for(i in 1:n){
        br_local <- t(rmaxstab(reps, s, "brown", nugget = 0, range = rhoW2rhoR(rho[i],1), smooth = 1))
        gp_local <- t(rgp(reps, s, "powexp", nugget = 0, range = rho[i], smooth = 1))
        br_exp   <- br_to_exp(br_local)
        gp_exp   <- gp_to_exp(gp_local)
        y        <- delta[i]*br_exp + (1-delta[i])*gp_exp
        y        <- phypoexp2(y, rate1 = 1/delta[i], rate2 = 1/(1-delta[i]))
